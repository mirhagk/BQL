...
sealed public class Rule_Root:Rule
{
  private Rule_Root(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_Root Parse(ParserContext context)
  {
    ...
    parsed = false;
    {
      ...
      if (parsed)
      {
        ...
        {
          rule = Rule_stmtList.Parse(context);
          if ((f1 = rule != null))
          {
    ...
    if (parsed)
    {
        rule = new Rule_Root(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
