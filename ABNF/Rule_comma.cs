...
sealed public class Rule_comma:Rule
{
  private Rule_comma(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_comma Parse(ParserContext context)
  {
    ...
    if (parsed)
    {
        rule = new Rule_comma(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
